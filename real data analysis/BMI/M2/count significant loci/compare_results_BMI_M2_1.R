setwd("/panfs/jay/groups/20/panwei/wan01299/collider_bias/BMI/MV_Egger/new/new_gwas/BMIMET/BMIMETPC/M2/second_turn/")
...
filename=c("resultDD1.txt","resultSH1.txt","resultEgger1.txt","resultLasso1.txt","resultMedian1.txt","resultMVMRcML1.txt","resultMVMRIVW1.txt")
Nmethod=length(filename)
print("here1")
outputname=numeric()

for(i in 1:Nmethod){
  outputname[i]=paste('Res_ldblockBMI_',filename[i],'BMI_M2_second_turn.RData',sep="")
}
result1=read.table(file="responseM2_1.BMI.glm.linear",header = FALSE)
