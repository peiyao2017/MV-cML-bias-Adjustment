...
for(i in 1:Ncov){
  ...
  filename_before[i]=paste("/panfs/jay/groups/20/panwei/wan01299/collider_bias/M2/Height/coefficient/responseM2_",i,".Height.glm.linear",sep="")
}
outputname=numeric()
for(i in 1:Ncov){
...
for(u in 1:Ncov){
  ...
  validation_small=read.table(file="Height_validation_small.txt",sep="\t",header=TRUE)
  validation_small=na.omit(validation_small)
  validation_large=read.table(file="Height_validation_large.txt",sep="\t",header=TRUE)
