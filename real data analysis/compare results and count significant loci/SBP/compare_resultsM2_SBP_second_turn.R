setwd("/panfs/jay/groups/20/panwei/wan01299/collider_bias/SBP")
...
for(i in 1:Ncov){
  ...
  filename_before[i]=paste("/panfs/jay/groups/20/panwei/wan01299/collider_bias/M2/SBP/coefficient/responseM2_",i,".SBP.glm.linear",sep="")
}
outputname=numeric()
for(i in 1:Ncov){
  outputname[i]=paste('Res_ldblockM2_second_turn',i,'.RData',sep="")
}
...
for(u in 1:Ncov){
  result1=read.table(file=filename_before[u],header = FALSE)
  ...
  result2=read.table(file=filename_after[u],header = TRUE)
  ...
  validation_small=read.table(file="SBP_validation_small.txt",sep="\t",header=TRUE)
  validation_small=na.omit(validation_small)
  validation_large=read.table(file="SBP_validation_large.txt",sep="\t",header=TRUE)
  ...
  if(nrow(sig_after)>0){
    ...
    Res = list(a1=a1,a2=a2,a3=a3,a4=a4,Nsnps_before=Nsnps_before,
               before=before,
  ...
  if(nrow(sig_after)==0){
    ...
    Res = list(a1=a1,a3=a3,a4=a4,Nsnps_before=Nsnps_before,
               before=before,
               validation_small_before=validation_small_before,
               validation_large_before=validation_large_before,
               Nsnps_after=Nsnps_after,
