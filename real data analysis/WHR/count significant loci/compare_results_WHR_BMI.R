...
library(R.utils)
Ncov=10
filename=c("resultMedian1WHR.txt","resultMVMRcML1WHR.txt","resultMVMRLasso1WHR.txt","resultMVMRIVW1WHR.txt","resultMVMREgger1WHR.txt","resultDDWHR.txt","resultSHWHR.txt")
Nmethod=length(filename)
outputname=numeric()
methods=c("MVMRMedian","MVMRcML","MVMRLasso","MVMRIVW","MVMREgger","DHO","SH")
for(i in 1:Nmethod){
  outputname[i]=paste('Res_ldblockBMI_',methods[i],'WHR1.RData',sep="")
}

result1=read.table(file="WHR_BMI.WHR.glm.linear",header = FALSE)
