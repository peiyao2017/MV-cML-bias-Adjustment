setwd("/panfs/jay/groups/20/panwei/wan01299/collider_bias/BMI/MV_Egger/new/no_bias")
...
for(t in 1:length(rhos)){
  ...
  for(t1 in 1:20){
    output=foreach(j=1:50,.combine = "rbind")%dopar%{
      ...
      for(i in 1:d){
        X[,i]=Gmatrix%*%betaGX[i,]+betaUX[i]*UX+EX[,i]
      }
 ...
 for(i in 1:ncol(correlation1)){
      ...
      for(i in 1:Nsnps){
        ...
        c1=sum(b^2*diag(covbetax))
        c2=t(b)%*%covbetax%*%b
        sdbetaGYadj1[i]=sqrt(a+b1+c1+sum(diag(covb*covbetax)))
      ...
      for(i in 1:Nsnps){
        covGtemp=covG[-i,-i]
        betaGXtemp=betaGX[,-i]
        M22total[[i]]=betaUX%*%t(betaUX)*var(UX)+diag(varEX)+betaGXtemp%*%covGtemp%*%t(betaGXtemp)
        print(i)
      }
      b_true=matrix(0,nrow = Nsnps,ncol = d)
      for(i in 1:Nsnps){
        b_true[i,]=-betaUY[1,1]*solve(M22total[[i]])%*%betaUX*var(UX)*0
      }
      slope=matrix(0,ncol=6*d,nrow=Nsnps)
      slope_se=matrix(0,ncol=6*d,nrow=Nsnps)
      for(i in 1:nrow(slope)){
        slope[i,]=c(b_true[i,],b,b_egger,b_ivw,b_lasso,b_median)
        slope_se[i,]=c(rep(0,times=d),sqrt(diag(covb)),sqrt(diag(covb_egger)),sqrt(diag(covb_ivw)),sqrt(diag(covb_lasso)),sqrt(diag(covb_median)))
      }
      ...
      colnames(slope)=c(name1,name2,name3,name4,name5,name6)
