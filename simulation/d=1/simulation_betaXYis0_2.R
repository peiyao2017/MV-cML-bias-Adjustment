setwd("/panfs/jay/groups/20/panwei/wan01299/collider_bias/BMI/MV_Egger/d=1/")
  ...
  for(t1 in 1:20){
    output=foreach(j=1:50,.combine = "rbind")%dopar%{
      ...
      repeat{
        ...
        Want=cml_MA(N=Nsample,p=ncol(betaGX1),m=nrow(betaGX_IV),betax=betaGX_IV,betayc=betaGYC_IV,SIG1=SIG,threshold=0.004,maxit=500)
      ...
      bias=numeric()
      bias_true=numeric()
      M22=solve(betaUX%*%t(betaUX)+ (varEX))
